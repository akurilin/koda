...
- The Supabase CLI is not installed on `PATH`. Invoke it via `npx supabase ...` (e.g. `npx supabase migration new <name>`, `npx supabase migration up`). Run from the repo root so it finds `supabase/config.toml`.
